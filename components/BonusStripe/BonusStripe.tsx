...
const BonusStripe: FunctionComponent<Props> = ({
  ...
  const visit = () => {
    ...
    window.open(
      `https://casinosquad.toply.info/visita/${bonus.compareCode}/${countryCode}`
    );
  };
  ...
  const extractLink = () => {
    ...
    const matchingLink = links.find(
      (it) =>
        it.label ===
        `${bonus.compareCode} ${configuration.streamerName} ${countryCode}`
    );
    if (matchingLink) return matchingLink.link;
    else {
      const toReturn = links.find(
        (it) =>
          it.label === `${bonus.compareCode} ${configuration.streamerName} row`
      );
